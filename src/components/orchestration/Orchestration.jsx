...
import { OrchestrationDetail } from "../../content/Data";
...
const Orchestration = ({ t }) => {
  return (
    <motion.div
      ...
      animate="visible"
    >
      {OrchestrationDetail.map((item, index) => (
        <motion.div
          ...
          <div className="flex gap-6 items-center md:flex-row flex-col">
            ...
            <div className="flex flex-col items-center md:items-start md:text-start text-center">
              <h2 className="text-2xl font-semibold uppercase text-white">
                {t(`orchestration.${item.title.toLowerCase()}.header`)}
              </h2>
              <p className="mt-2">
                {t(`orchestration.${item.title.toLowerCase()}.body`)}
              </p>
            </div>
          </div>
        </motion.div>
